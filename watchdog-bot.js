 ...
 *   /restart-windows - Restart Windows Clawdbot
 *   /restart-wsl - Restart WSL Clawdbot
 */
...
const CONFIG = {
  ...
  wslScriptPath: process.env.WSL_RESTART_SCRIPT || '~/clawdbot-watchdog/restart.sh'
};
...
function checkWslGateway(port) {
  ...
  return new Promise((resolve) => {
    const cmd = 'wsl -e bash -l -c "clawdbot gateway health 2>&1 | grep -q \\"OK\\" && echo OK || echo FAIL"';
    exec(cmd, { timeout: 10000 }, (err, stdout) => {
      if (err) resolve(false);
      else resolve(stdout.trim() === 'OK');
    });
...
function runWindowsRestart() {
  return new Promise((resolve) => {
    ...
    const child = spawn(
      'cmd.exe',
      ['/c', 'start', '""', '/min', 'clawdbot', 'gateway', 'run'],
...
function buildWslRestartCommand() {
  ...
  const winScript = path.resolve(__dirname, 'wsl-clawdbot-restart.sh');
...
bot.onText(/\/restart-windows/, async (msg) => {
  if (!isAuthorized(msg)) return;
  const restartMsg = await bot.sendMessage(msg.chat.id, '🔄 Restarting Windows Clawdbot...');
  ...
  try {
    ...
    if (isOnline) {
      bot.sendMessage(msg.chat.id, '✅ Windows Clawdbot restarted successfully!');
    } else {
...
bot.onText(/\/restart-wsl/, async (msg) => {
  ...
  await bot.sendMessage(msg.chat.id, '🔄 Restarting WSL Clawdbot...');
  ...
  try {
    ...
    if (isWslOnline) {
      bot.sendMessage(msg.chat.id, '✅ WSL Clawdbot restarted successfully!');
    } else {
