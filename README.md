...
A small Telegram bot that monitors the Clawdbot Gateway and can restart it on Windows
and WSL.
...
- Clawdbot installed and available on PATH (Windows and/or WSL)
...
```
Please set up the Clawdbot Watchdog in this repo folder.
...
- The WSL restart script is .\wsl-clawdbot-restart.sh
```
...
- WSL_RESTART_SCRIPT: defaults to ~/clawdbot-watchdog/restart.sh
...
CHAT_ID=123456789
GATEWAY_PORT=18789
WSL_RESTART_SCRIPT=~/clawdbot-watchdog/restart.sh
```
...
```
.\wsl-clawdbot-restart.sh -> ~/clawdbot-watchdog/restart.sh
```
...
- If WSL restart fails, ensure Clawdbot is installed in WSL and restart.sh is executable.
